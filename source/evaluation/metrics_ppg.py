...
def calculate_metrics_ppg(predictions, labels, config, evaluation_file_path, used_epoch, metrics):
    ...
    for participant in tqdm(predictions.keys(), ncols=80):
        ...
        for i in range(0, len(prediction), window_frame_size):
            ...
            if config.TEST.DATA.PREPROCESS.LABEL_TYPE in ['Diff', 'DiffStandardized', 'DiffStandardizedExtended']:
                ...
                diff_flag = False
            gt_hr, pred_hr, SNR, MSE = calculate_metric_per_video_ppg(
        ...
        if config.TEST.DATA.DATASET == 'egoexo4d':
            np.save(f'/data/bjbraun/Projects/egoPPG/predicted_hrs/egoexo4d/{participant}_hrs.npy',
                    np.asarray(resample_signal(pred_part_save, prediction.shape[0], 'linear')))
            np.save(f'/data/bjbraun/Projects/egoPPG/predicted_rppgs/egoexo4d/{participant}_rppgs.npy',
                    np.asarray(prediction))

            """fig, ax = plt.subplots()
            ax.plot(pred_part_save, label='Predicted HR')
            ax.set_title(f'{participant}: Predicted HR')
            fig.show()"""
