...
from utils import get_task_chunk_list, resample_signal
...
def calculate_metrics_ppg(predictions, labels, config, signal_name, evaluation_file_path, used_epoch, metrics,
                             i_label):
    ...
    for participant in tqdm(predictions.keys(), ncols=80):
        ...
        if config.TEST.DATA.DATASET in ['egoexo4d']:
            if signal_name == 'ppg':
                # If dir does not exist, create it
                if not os.path.exists(f'{config.LOG.PATH_PREDS}/predicted_hrs/{config.MODEL.NAME}/'
