...
import math
from source.ml.models.PhysFormer import ViT_ST_ST_Compact3_TDC_gra_sharp
...
#############################################
class PhysFormerDiffusionBase(nn.Module):
    """
    Two-stage model:
    ...
    def __init__(self, image_size, patches, dim, ff_dim, num_heads, num_layers, dropout_rate, theta,
                 diffusion_steps: int = 50, device: str = "cpu", beta_schedule="cosine"):
        super().__init__()
        self.PhysFormer = ViT_ST_ST_Compact3_TDC_gra_sharp(image_size=image_size, patches=patches, dim=dim,
                                                           ff_dim=ff_dim, num_heads=num_heads, num_layers=num_layers,
                                                           dropout_rate=dropout_rate, theta=theta)
    ...
    def forward(self, vid, gra_sharp, denoiser, *,
        compute_diff_loss: bool = False,
        signal_norm: bool = True,     # normalise PhysNet output to ±1
        ...
        # 1) PhysNet forward
        # -------------------------------------------------------------- #
        # raw, _, _, _ = self.PhysFormer(vid, gra_sharp)           # [B,L]
        ...
        # -------------------------------------------------------------- #
        # 3) Compute ε-prediction loss (only if requested)
        # -------------------------------------------------------------- #
        if compute_diff_loss:
            eps_pred  = denoiser(x_t, t_rand)        # CNNDenoiser embeds t internally
            diff_loss = F.mse_loss(eps_pred, eps) * diff_loss_weight
        else:
            diff_loss = None

        # -------------------------------------------------------------- #
        # 4) Reverse loop (inference only)
        # -------------------------------------------------------------- #
...
class PhysFormerDiffusionCNN(nn.Module):
    def __init__(self, image_size, patches, dim, ff_dim, num_heads, num_layers, dropout_rate, theta,
                 diffusion_steps=50, device='cpu'):
        super(PhysFormerDiffusionCNN, self).__init__()
        self.base = PhysFormerDiffusionBase(image_size, patches, dim, ff_dim, num_heads, num_layers, dropout_rate, theta,
                                            diffusion_steps=diffusion_steps, device=device)
        self.cnn_denoiser = CNNDenoiser()

    def forward(self, x, gra_sharp, compute_diff_loss=False):
        return self.base(x, gra_sharp, self.cnn_denoiser, compute_diff_loss=compute_diff_loss)


# Joint model variant with U-Net denoiser
class PhysFormerDiffusionUNet(nn.Module):
    def __init__(self, image_size, patches, dim, ff_dim, num_heads, num_layers, dropout_rate, theta,
                 diffusion_steps=50, device='cpu'):
        super(PhysFormerDiffusionUNet, self).__init__()
        self.base = PhysFormerDiffusionBase(image_size, patches, dim, ff_dim, num_heads, num_layers, dropout_rate, theta,
                                            diffusion_steps=diffusion_steps, device=device)
        self.unet_denoiser = UNetDenoiser()

    def forward(self, x, gra_sharp, compute_diff_loss=False):
        return self.base(x, gra_sharp, self.unet_denoiser, compute_diff_loss=compute_diff_loss)
