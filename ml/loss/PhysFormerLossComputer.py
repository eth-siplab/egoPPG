...
class TorchLossComputer(object):
    ...
    @staticmethod
    def cross_entropy_power_spectrum_DLDL_softmax2(inputs, target, Fs, std, device, min_hr_rr, max_hr_rr):
        target_distribution = [normal_sampling(int(target), i, std) for i in range(min_hr_rr, max_hr_rr)]
        ...
        whole_max_idx = whole_max_idx.type(torch.float)
        return (loss_distribution_kl, F.cross_entropy(ca, (target - bpm_range[0]).view(1).type(torch.long)),
                torch.abs(target[0] - bpm_range[0] - whole_max_idx))
