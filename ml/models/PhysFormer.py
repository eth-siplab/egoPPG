...
class ViT_ST_ST_Compact3_TDC_gra_sharp(nn.Module):
    def __init__(
        ...
        # ToDo: Changed 3 to 1 for monochrome input
        self.Stem0 = nn.Sequential(
            nn.Conv3d(1, dim // 4, [1, 5, 5], stride=1, padding=[0, 2, 2]),
    ...
    def forward(self, x, gra_sharp):
        ...
        x = self.Stem0(x)  # [b, 24, t, h/2, w/2]
        x = self.Stem1(x) # [b, 48, t, h/4, w/4]
        x = self.Stem2(x)  # [B, 64, 160, 64, 64]  # [b, 64, t, h/8, w/8]

        x = self.patch_embedding(x)  # [B, 64, 40, 4, 4]  # [b, 96, t/4, h/32, w/32]  = [4, 96, 32, 1, 4]
        ...
        # upsampling heads
        # ToDo: Changed 4 to 1 for input height of 48
        # features_last = Trans_features3.transpose(1, 2).view(b, self.dim, 40, 4, 4) # [B, 64, 40, 4, 4]
        features_last = Trans_features3.transpose(1, 2).view(b, self.dim, t // 4, 1, 4)  # [B, 64, 40, 4, 4]
