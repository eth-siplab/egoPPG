...
from source.ml.models.DiffusionDenoiserPost import (DiffusionDenoiserPost, diffusion_denoising,
                                                    PostProcessingDiffusionUNet, PostProcessingDiffusionTransformer)
from source.ml.loss.FreqLossIMU import FreqLossIMU
from source.ml.loss.FocalLoss import FocalLoss
from source.ml.models.PhysNetNewIMUCATWO import PhysNetNewIMUCATWO
from source.ml.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


def generate_soft_labels(targets, num_classes, sigma=2.0):
    """
    Generate soft labels for ordinal regression.

    Args:
        targets (torch.Tensor): Ground truth HR class indices of shape [B].
    ...
    B = targets.size(0)
    # Create a tensor with class indices [0, 1, ..., num_classes-1]
    class_indices = torch.arange(num_classes, device=device).float()  # Shape: [num_classes]
    # Expand to shape [B, num_classes]
    class_indices = class_indices.unsqueeze(0).expand(B, num_classes)
    # Expand targets to shape [B, 1]
    targets = targets.unsqueeze(1).float()
    # Compute the Gaussian probability for each class value
...
class PhysNetNewIMUCATWOTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        self.model = PhysNetNewIMUCATWO(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH,
                                        num_embeddings=self.num_embeddings).to(self.device)
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            if self.config.LABEL_SIGNALS[0] in ['ppg_finger', 'ppg_ear', 'ppg_nose']:
                self.loss_model = FreqLossIMU('ppg', config.TRAIN.DATA.FS, self.device)
            elif self.config.LABEL_SIGNALS[0] in ['rr']:
                self.loss_model = FreqLossIMU('rr', config.TRAIN.DATA.FS, self.device)
            else:
                raise ValueError("Label signal error! Please check label signal.")
            self.hr_loss_model = torch.nn.CrossEntropyLoss()
            # self.hr_loss_model = FocalLoss(alpha=1.0, gamma=2.0)
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                pred, class_logits = self.model(batch[0][0].to(self.device), batch[0][1].to(self.device))
                ...
                signal_loss = self.loss_model(pred, label, batch[0][1].to(self.device))
                # hr_loss = F.cross_entropy(class_logits, batch[1][1].to(self.device))
                # hr_loss = self.focal_loss(class_logits, batch[1][1].to(self.device))
                ...
                # hr_loss = self.hr_loss_model(class_logits, target_class)

                soft_labels = generate_soft_labels(mean_gt_hr, self.num_embeddings, 2.0)
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for _, batch in enumerate(tbar):
                ...
                pred, class_logits = self.model(batch[0][0].to(self.config.DEVICE), batch[0][1].to(self.device))
