...
def ICA(imu, FS, LPF, HPF):

    NyquistF = 1 / 2 * FS
    nc = imu.shape[1]
    for c in range(nc):
        imu[:, c] = signal.detrend(imu[:, c])
    _, S = ica(np.mat(imu).H, nc)
    ...
    BVP_I = signal.filtfilt(B, A, np.real(BVP_I).astype(np.double))
    # BVP_I = np.asarray(np.real(BVP_I).astype(np.double))
