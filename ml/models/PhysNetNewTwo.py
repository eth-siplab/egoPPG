...
class PhysNetNewTwo(nn.Module):
    def __init__(self, frames=128):
        super(PhysNetNewTwo, self).__init__()
        ...
        self.ConvBlockApp = nn.Sequential(
            nn.Conv3d(1, 16, (1, 5, 5), stride=1, padding=(0, 2, 2)),
            nn.BatchNorm3d(16),
            nn.ReLU(inplace=True),
        )
        self.appearance_att_conv = nn.Conv3d(16, 1, kernel_size=1, padding=(0, 0, 0), bias=True)
        ...
        # self.SpatialAttentionGateApp = SpatialAttentionGate()
    ...
    def forward(self, x):  # Batch_size*[3, T, 128,128]
        x_mot = x[:, 0:1, :, :, :]
        x_app = x[:, 1:2, :, :, :]
        [batch, channel, length, height, width] = x_mot.shape

        # transform the input
        # x = self.stn(x.view(batch * length, channel, height, width)).view(batch, channel, length, height, width)
        # x_scale = self.SpatialAttentionGate1(x_app.reshape(batch * length, channel, height, width))
        # x = (x_mot.reshape(batch * length, channel, height, width) * x_scale).view(batch, channel, length, height, width)
        # x = self.SpatialAttentionGate1(x)

        # Motion branch
        x = self.ConvBlock1(x_mot)  # x [16, T, 128,128]
        x = self.SpatialAttentionGate1(x.view(batch * length, 16, height, width)).view(batch, 16, length, height, width)

        # Appearance branch
        x_app = self.ConvBlockApp(x_app)  # x [16, T, 128,128]
        x_app = torch.sigmoid(self.appearance_att_conv(x_app))
        # x_att = self.SpatialAttentionGateApp(x_app.view(batch * length, 1, height, width)).view(batch, 1, length, height, width)
        x_att = self.attn_mask(x_app)
        x = x * x_att
