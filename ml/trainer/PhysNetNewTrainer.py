...
class PhysNetNewTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        self.model = PhysNetNew(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH, in_channels=3).to(self.device)
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            """if self.config.LABEL_SIGNALS[0] in ['ppg_finger', 'ppg_ear', 'ppg_nose', 'ppg']:
