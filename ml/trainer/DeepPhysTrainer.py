...
from source.ml.models.DeepPhys import DeepPhys
...
class DeepPhysTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        self.model = DeepPhys(w=config.TRAIN.DATA.PREPROCESS.RESIZE.W, h=config.TRAIN.DATA.PREPROCESS.RESIZE.H,
                              in_channels=1).to(self.device)
    ...
    def valid(self, data_loader):
        ...
        valid_step = 0
        with torch.no_grad():
            if self.config.LABEL_SIGNALS[self.config.LABEL_VALID] in ['ppg_finger', 'ppg_ear', 'ppg_nose', 'rr']:
        ...
        with torch.no_grad():
            for _, batch in enumerate(data_loader['test']):
                ...
                for idx in range(batch_size):
                    ...
                    predictions[self.config.LABEL_SIGNALS[0]][subj_index][sort_index] = pred[idx * chunk_len_test:(idx + 1) * chunk_len_test]
                    labels[self.config.LABEL_SIGNALS[0]][subj_index][sort_index] = label[idx * chunk_len_test:(idx + 1) * chunk_len_test]
