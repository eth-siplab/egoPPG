...
# from source.evaluation.post_process import calculate_hr, calculate_psd
...
class Hybrid_Loss(nn.Module):
    ...
    def forward(self, pred_ppg, labels, epoch, FS, diff_flag, device):
        ...
        loss_Fre, _ = TorchLossComputer.Frequency_loss(pred_ppg.squeeze(-1), labels.squeeze(-1), diff_flag, FS, 3.0,
                                                       device)
...
class RhythmFormer_Loss(nn.Module):
    ...
    def forward(self, pred_ppg, labels, epoch, FS, diff_flag):
        loss_time = self.criterion_Pearson(pred_ppg.view(1, -1), labels.view(1, -1))
        loss_CE, loss_distribution_kl = TorchLossComputer.Frequency_loss(pred_ppg.squeeze(-1), labels.squeeze(-1),
                                                                         diff_flag=diff_flag, Fs=FS, std=3.0)
        loss_hr = TorchLossComputer.HR_loss(pred_ppg.squeeze(-1), labels.squeeze(-1), diff_flag=diff_flag, Fs=FS,
                                            std=3.0)
        if torch.isnan(loss_time):
            loss_time = 0
...
class PhysFormer_Loss(nn.Module):
    ...
    def forward(self, pred_ppg, labels, epoch, FS, diff_flag):
        ...
        loss_CE, loss_distribution_kl = TorchLossComputer.Frequency_loss(pred_ppg.squeeze(-1), labels.squeeze(-1),
                                                                         diff_flag=diff_flag, Fs=FS, std=1.0)
...
class TorchLossComputer(object):
    ...
    @staticmethod
    def complex_absolute(output, Fs, bpm_range, device):
    ...
    @staticmethod
    def cross_entropy_power_spectrum_forward_pred(inputs, Fs, device):
        inputs = inputs.view(1, -1)
        bpm_range = torch.arange(40, 190, dtype=torch.float).cuda()
        # bpm_range = torch.arange(40, 180, dtype=torch.float).cuda()
        # bpm_range = torch.arange(40, 260, dtype=torch.float).cuda()

        complex_absolute = TorchLossComputer.complex_absolute(inputs, Fs, bpm_range, device)
    ...
    @staticmethod
    def Frequency_loss(inputs, target, diff_flag, Fs, std, device):
        ...
        ca = TorchLossComputer.complex_absolute(inputs, Fs, bpm_range, device)
        sa = ca / torch.sum(ca)
    ...
    @staticmethod
    def HR_loss(inputs, target, diff_flag, Fs, std, device):
