...
class Fusion_Stem(nn.Module):
    def __init__(self, apha=0.5, belta=0.5, dim=24, in_channels=3):
        super(Fusion_Stem, self).__init__()

        self.stem11 = nn.Sequential(nn.Conv2d(in_channels, dim // 2, kernel_size=7, stride=2, padding=3),
        ...
        self.stem12 = nn.Sequential(nn.Conv2d(4 * in_channels, dim // 2, kernel_size=7, stride=2, padding=3),
    ...
    def forward(self, x):
        ...
        x_diff = self.stem12(torch.cat([x2 - x1, x3 - x2, x4 - x3, x5 - x4], 2).view(N * D, 4*C, H, W))
...
class RhythmMamba(nn.Module):
    def __init__(self,
        ...
        self.Fusion_Stem = Fusion_Stem(dim=embed_dim // 4, in_channels=in_channels)
