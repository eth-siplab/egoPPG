...
class CrossAttentionIMUImageold(nn.Module):
    ...
    def forward(self, imu_data, rppg_frames):
        ...
        rppg_frames = rppg_frames.view(batch_size * seq_len, self.feature_extractor_dim_in, h, w)  # Reshape for CNN processing (4 * 128, 1, 48, 128)
        img_features = self.feature_extractor(rppg_frames)  # Shape: (batch_size*sequence_length, 32, 8*8)
        img_features = img_features.view(batch_size, seq_len, self.embedding_dim)  # Shape: (batch_size, sequence_length, feature_dim)
        ...
        # attn_output = attn_output + imu_embedded  # Residual
...
class PhysNetNewIMUCA(nn.Module):
    def __init__(self, frames=128, h=48, w=128, num_embeddings=141):
        ...
        self.cross_attention = CrossAttentionIMUImage(embedding_dim=128, num_heads=4, feature_extractor_dim_in=1,
                                                      feature_extractor_dim_out=32)  # ToDo
