...
from source.ml.models.PhysNetIMU import PhysNetIMU, DeepConvLSTM, DeepConvLSTMBe
from source.ml.models.PhysNetNew import PhysNetNew
from source.ml.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


class PhysNetIMUTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        # Load pretrained HR model
        self.model_pre = PhysNetNew(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH).to(self.device)
        for param in self.model_pre.parameters():  # freeze model
            param.requires_grad = False
        config_pre = ('/egoppg/inputs_et/labels_ppg_nose/'
                      'CL128_Down1_W128_H48_LabelDiffStandardized_VideoTypeDiffStandardized'
                      '/PhysNetNew/seed_0')
        model_dir_pre = f'/data/bjbraun/Projects/egoPPG/PreTrainedModels/{config_pre}'
        log_training_dir_pre = f'/local/home/bjbraun/Projects/egoPPG/runs/training/{config_pre}'
        model_path_pre, used_epoch_pre = self.get_model_path(self.config, model_dir_pre, log_training_dir_pre,
                                                             self.cv_split, self.max_epoch_num, self.best_epoch)
        self.model_pre.load_state_dict(torch.load(model_path_pre, weights_only=True))

        # self.model = PhysNetIMU(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH).to(self.device)
        self.model = DeepConvLSTMBe(2).to(self.device)
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                ppg = self.model_pre(batch[0][0].to(self.device))
                input_model = torch.cat((ppg.view(ppg.shape[0], 1, ppg.shape[1]), batch[0][1].to(self.device).
                                         view(batch[0][1].shape[0], 1, batch[0][1].shape[1])), dim=1)
                # input_model = ppg.view(ppg.shape[0], 1, ppg.shape[1])
                pred = self.model(input_model)
                ...
                label = torch.mean(label, dim=1).view(-1, 1)  # ToDo
                ...
                # torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0)  # ToDo
            ...
            if self.config.LABEL_SIGNALS[self.config.LABEL_VALID] in ['ppg_finger', 'ppg_ear', 'ppg_nose', 'hr']:
                ...
                for idx, batch in enumerate(vbar):
                    ...
                    if self.config.LABEL_SIGNALS[0] != 'hr':
                        label = (label - torch.mean(label)) / torch.std(label)  # normalize
                        pred = (pred - torch.mean(pred)) / torch.std(pred)  # normalize
                    label = torch.mean(label, dim=1).view(-1, 1)
                    loss = self.loss_model(pred, label)
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, batch in enumerate(data_loader['test']):
                ...
                # input_model = ppg.view(ppg.shape[0], 1, ppg.shape[1])
                pred = self.model(input_model)
                # pred = pred.repeat(1, 128)  # Shape becomes (batch_size, 128)
        ...
        for label_signal in self.config.LABEL_SIGNALS:
            ...
            elif label_signal in ['hr']:
                results[label_signal] = calculate_metrics_hr(predictions, labels, self.config, self.log_testing_dir +
                                                             f'/cv{self.cv_split}_Evaluation.txt', used_epoch,
                                                             self.test_metrics[label_signal])
