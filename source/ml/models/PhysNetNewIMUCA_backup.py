...
class CrossAttentionIMUImage(nn.Module):
    ...
    def forward(self, imu_data, rppg_frames):
        ...
        rppg_frames = rppg_frames.view(batch_size * seq_len, 1, h, w)  # Reshape for CNN processing (4 * 128, 1, 48, 128)
        img_features = self.feature_extractor(rppg_frames)  # Shape: (batch_size*sequence_length, 32, 8*8)
        img_features = img_features.view(batch_size, seq_len, -1)  # Shape: (batch_size, sequence_length, feature_dim)
...
class PhysNetNewIMUCA(nn.Module):
    def __init__(self, frames=128, h=48, w=128):
        super(PhysNetNewIMUCA, self).__init__()
        ...
        # self.temporal_attention = TemporalAttentionGate(nt=frames)

        n_embed = 64
        self.cross_attention = CrossAttentionIMUImage(embedding_dim=n_embed, num_heads=4)
        self.patch_embed = PatchEmbed(img_size=(w, h), patch_size=16, in_chans=1, embed_dim=n_embed)

    def forward(self, x, imu):  # Batch_size*[3, T, 128,128]
        [batch, channel, length, height, width] = x.shape

        test = self.patch_embed(x)

        # IMU
        # imu = self.conv_imu_3axis(imu.permute(0, 2, 1)).permute(0, 2, 1).squeeze()
        imu = self.norm(imu)
        imu = imu.unsqueeze(-1)

        # Image
        output = self.cross_attention(imu, x)
