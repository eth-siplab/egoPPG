...
from source.evaluation.metrics_ppg import _reform_data_from_dict
...
def calculate_metrics_bp(predictions, labels, config, evaluation_file_path, used_epoch, ax_all, metrics):
    SC_all = list()
    MAE_all = list()
    ...
    for index in tqdm(predictions.keys(), ncols=80):
        ...
        if index == list(predictions.keys())[0]:
