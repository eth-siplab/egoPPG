...
class FreqLossIMU(nn.Module):
    def __init__(self, signal_type, fs, device, alpha=1.0):
        super(FreqLossIMU, self).__init__()
        if signal_type == 'ppg':
            L = 512
        elif signal_type == 'rr':
            L = 512
        ...
        self.device = device
        self.alpha = alpha
        self.neg_pear_loss = Neg_Pearson()
        return

    def forward(self, pred, label, imu_mag):
        ...
        # motion_weight = torch.exp(-self.alpha * imu_mag)  # shape: [batch_size]
        # Average over the batch (or you could apply it per sample if you adapt the loss terms)
        # motion_weight = torch.mean(motion_weight)
        ...
        # return mse + motion_weight * (l2 + l3)
        # return mse + 1.0 * (l2+l3)
