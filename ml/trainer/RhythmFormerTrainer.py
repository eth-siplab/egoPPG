...
from source.ml.models.RhythmFormer import RhythmFormer
from source.ml.trainer.BaseTrainer import BaseTrainer
from source.ml.loss.RythmFormerLossComputer import RhythmFormer_Loss
...
class RhythmFormerTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            if config.LABEL_SIGNALS[0] == 'rr':
                self.criterion = RhythmFormer_Loss('rr')
            else:
                self.criterion = RhythmFormer_Loss('ppg')
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                loss = 0.0
                for ib in range(N):
                    loss = loss + self.criterion(pred[ib], label[ib], epoch, self.config.TRAIN.DATA.FS,
                                                 self.diff_flag, self.device, self.min_hr_rr, self.max_hr_rr)
                loss = loss / N
            ...
            if self.config.LABEL_SIGNALS[self.config.LABEL_VALID] in ['ppg_finger', 'ppg_ear', 'ppg_nose', 'hr', 'rr']:
                ...
                for idx, batch in enumerate(vbar):
                    ...
                    if self.config.LABEL_SIGNALS[0] != 'hr':
                        ...
                        pred = (pred - torch.mean(pred)) / torch.std(pred)  # normalize
                    for ib in range(N):
                        loss = self.criterion(pred[ib], label[ib], self.config.TRAIN.EPOCHS, self.config.TRAIN.DATA.FS,
                                              self.diff_flag, self.device, self.min_hr_rr, self.max_hr_rr)
