...
def mp_main(participant, configs, win_size_hr, stride_hr, low_pass, high_pass, methods, tasks_evaluate):
    ...
    for i_win in range(n_wins):
        ...
        start_ppg_shimmer = i_win * stride_hr * fs_all['shimmer_ppg']
        ...
        if participant not in configs['exclusion_list_shimmer']:
            shimmer_ppg_temp = shimmer_ppg[start_ppg_shimmer:start_ppg_shimmer + win_size_hr * fs_all['shimmer_ppg']]
        ...
        if participant not in configs['exclusion_list_shimmer']:
            shimmer_ppg_temp = filter_ppg(shimmer_ppg_temp, fs_all['shimmer_ppg'], 'Shimmer')
        ...
        if 'rppg_peaks' in methods and 'rppg_fft' in methods:
        ...
        if 'ppg_s_peaks' in methods_part and 'ppg_s_fft' in methods_part:
            ppg_hr_s_peaks = calculate_peak_hr(shimmer_ppg_temp, fs_all['shimmer_ppg'])
            ppg_hr_s_fft = calculate_fft_hr(shimmer_ppg_temp, fs_all['shimmer_ppg'], acc_magn=aria_acc_mag_temp,
        ...
        ibis_ecg = calculate_ibis(info_ecg['ECG_R_Peaks'], fs_all['ms_ecg'], 1.5, reject_outliers=True)
