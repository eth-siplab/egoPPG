...
from source.ml.models.DiffusionPhys import DiffusionPhys
from source.ml.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


class DiffusionPhysTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        self.model = DiffusionPhys(diffusion_steps=50, device=self.device, beta_schedule="cosine").to(self.device)
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                reconstruction_loss = self.loss_model(pred, label)
                loss = lambda1 * reconstruction_loss + lambda2 * diffusion_loss
