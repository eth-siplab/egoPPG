...
from source.ml.models.VQVAE import VQVAE, HierarchicalVQVAE
from source.ml.loss.FreqLoss import FreqLoss, calculate_2_wasserstein_dist
from source.ml.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


def spectral_loss(pred, target):
    # pred, target: [B, signal_length]
...
class VQVAETrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        self.model = VQVAE(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH,
                           out_length=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH,
                           num_embeddings=self.num_embeddings,
                           commitment_cost=0.5).to(self.device)
        """self.model = HierarchicalVQVAE(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH,
                                       out_length=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH,
                                       num_embeddings_low=self.num_embeddings,
                                       commitment_cost=0.5).to(self.device)"""
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # loss = 0.5 * signal_loss + 1 * hr_loss + 4 * vq_loss

                """lambda_recon = 1.0
                lambda_spec = 0.5
                lambda_class = 1.0
                spec_loss = spectral_loss(pred, label)
                loss = lambda_recon * signal_loss + lambda_spec * spec_loss + lambda_class * hr_loss + vq_loss"""
