...
from source.ml.models.FactorizePhys.FSAM import FeaturesFactorizationModule
...
class BVP_Head(nn.Module):
    def __init__(self, md_config, device, dropout_rate=0.1, debug=False):
        ...
        self.final_layer = nn.Sequential(
            ConvBlock3D(inC, nf[1], [3, 3, 3], [1, 1, 1], [1, 1, 1]),                         #B, nf[1], 160, 5, 5
            ConvBlock3D(nf[1], nf[0], [3, 3, 3], [1, 1, 1], [1, 1, 1]),                       #B, nf[0], 160, 3, 3
            nn.Conv3d(nf[0], 1, (3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1), bias=False),  #B, 1, 160, 1, 1
            nn.AvgPool3d(kernel_size=(1, 1, 21))  # for egocentric vision width dimension
        )
