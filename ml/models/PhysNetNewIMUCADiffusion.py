...
class PhysNetDiffusionBase(nn.Module):
    ...
    def __init__(self, diffusion_steps: int = 50, frames: int = 128, device: str = "cpu", beta_schedule="cosine"):
        super().__init__()
        self.physnet = PhysNetNewIMUCA(frames=frames)
    ...
    def forward(self, vid, imu, denoiser, *,
...
class PhysNetNewIMUCADiffusionCNN(nn.Module):
    def __init__(self, diffusion_steps=50, frames=128, device='cpu'):
        super(PhysNetNewIMUCADiffusionCNN, self).__init__()
        self.base = PhysNetDiffusionBase(diffusion_steps=diffusion_steps, device=device, frames=frames)
        self.cnn_denoiser = CNNDenoiser()

    def forward(self, x, imu, compute_diff_loss=False):
        return self.base(x, imu, self.cnn_denoiser, compute_diff_loss=compute_diff_loss)


# Joint model variant with U-Net denoiser
class PhysNetNewIMUCADiffusionUNet(nn.Module):
    def __init__(self, diffusion_steps=50, frames=128, device='cpu'):
        super(PhysNetNewIMUCADiffusionUNet, self).__init__()
        self.base = PhysNetDiffusionBase(diffusion_steps=diffusion_steps, device=device, frames=frames)
        self.unet_denoiser = UNetDenoiser()

    def forward(self, x, imu, compute_diff_loss=False):
        return self.base(x, imu, self.unet_denoiser, compute_diff_loss=compute_diff_loss)
