...
from source.ml.loss.VRPMSLoss import FRL,FAL,FCL, mse_loss_VRMPS
from source.ml.models.VRPMS import VRPMS
from source.ml.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


class VRPMSTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        self.model = VRPMS(64, 4, config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH, 4, self.device,
                           config.TRAIN.DATA.PREPROCESS.RESIZE.W, config.TRAIN.DATA.PREPROCESS.RESIZE.H,
                           in_channels=3,).to(self.device)
        if len(device_ids_list) > 1:
            self.model = torch.nn.DataParallel(self.model, device_ids=device_ids_list)

        if config.TOOLBOX_MODE == "train_and_test":
            # self.loss_model = Neg_Pearson()
            self.criterion2 = FRL(Fs=config.TRAIN.DATA.FS, min_hr=40, max_hr=180)
            # self.criterion3 = FAL(Fs=config.TRAIN.DATA.FS, high_pass=3.0, low_pass=0.4)
            # self.criterion4 = FCL(Fs=config.TRAIN.DATA.FS, high_pass=3.0, low_pass=0.4)
            self.criterion3 = FAL(Fs=config.TRAIN.DATA.FS, high_pass=40, low_pass=250)
            self.criterion4 = FCL(Fs=config.TRAIN.DATA.FS, high_pass=40, low_pass=250)
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                input_temp, positive1 = batch[0][0].to(self.config.DEVICE), batch[0][1].to(self.config.DEVICE)
                positive2, neighbor1 = batch[0][2].to(self.config.DEVICE), batch[0][3].to(self.config.DEVICE)
                neighbor2, neighbor3 = batch[0][4].to(self.config.DEVICE), batch[0][5].to(self.config.DEVICE)
                ratio_array = batch[0][6].to(self.config.DEVICE)
                neg_rppgarr, pos_rppg1, pos_rppg2, neighbor_rppg1, neighbor_rppg2, neighbor_rppg3, negative_arr \
                    = self.model(input_temp, positive1, positive2, neighbor1, neighbor2, neighbor3, ratio_array)

                l_rec = mse_loss_VRMPS(input_temp, negative_arr)
                l_frl = self.criterion2(neg_rppgarr, pos_rppg1, pos_rppg2, ratio_array)
                l_fal = self.criterion3(pos_rppg1, pos_rppg2, neighbor_rppg1, neighbor_rppg2, neighbor_rppg3)
                l_fcl = self.criterion4(neg_rppgarr, pos_rppg1, pos_rppg2)
