...
class ContrastDatasetLoader(Dataset):
    ...
    def __init__(self, dataset_name, cv_split_files, input_signals, label_signals, config_data, no_labels=False):
        ...
        self.index_list = [i for i in range(len(self.inputs_list[0]))]
    ...
    def __getitem__(self, index):
        ...
        # while True:
        index2 = random.randrange(len(self.inputs_list[0]))
        # if self.inputs_list[0][index2].split('/'[-1]) != pid1:  # enforce *different* subject
        #     break
        ...
        for i_input in range(len(self.inputs_list)):
            ...
            if self.dataset_name in ['Train', 'Valid']:
                data_temp2 = torch.from_numpy(np.load(self.inputs_list[i_input][index2]))
                if self.input_signals[i_input] in ['face', 'hand', 'et']:
                    if self.data_format == 'NDCHW':
                        data_temp2 = data_temp2.permute(1, 0, 2, 3)
                else:
                    data_temp2 = data_temp2.type(torch.float32)

                pair = torch.stack((data_temp, data_temp2), dim=0)
            else:
                pair = data_temp
            data_out.append(pair)

        labels_out = []
        if not self.no_labels:  # NEW
            for i_label in range(len(self.labels_list)):
                lab1 = np.float32(np.load(self.labels_list[i_label][index]))

                if self.dataset_name in ['Train', 'Valid']:
                    lab2 = np.float32(np.load(self.labels_list[i_label][index2]))
                    labels_out.append(np.vstack([lab1, lab2]))
                else:
                    labels_out.append(lab1)
