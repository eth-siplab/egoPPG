...
from source.ml.models.DiffusionDenoiserPost import PostProcessingDiffusionUNet, PostProcessingDiffusionTransformer
from source.ml.models.PhysNetNewDiffusion import PhysNetDiffusionUNet, PhysNetDiffusionTransformer, PhysNetDiffusionCNN
from source.ml.models.PhysNetNew import PhysNetNew
from source.ml.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


class PhysNetNewDiffusionTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        # self.model = PhysNetNew(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH).to(self.device)
        self.model = PhysNetDiffusionUNet(diffusion_steps=50, frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH, device=self.device).to(self.device)
        # self.model = PhysNetDiffusionTransformer(frames=config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH, diffusion_steps=50, device=self.device).to(self.device)
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            # self.loss_model = torch.nn.L1Loss()
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                pred, diffusion_loss = self.model(batch[0][0].to(self.device), compute_diff_loss=True)
                ...
                self.scheduler.step()
                tbar.set_postfix(loss=total_loss.item())
    ...
    def test(self, data_loader):
        ...
        # Instantiate diffusion model (for demonstration, using untrained weights)
        ...
        with torch.no_grad():
            ...
            for _, batch in enumerate(tbar):
                ...
                # n_frames += batch[0][0].shape[0] * batch[0][0].shape[2]
