...
from source.ml.loss.ContrastLoss import ContrastLoss, IrrelevantPowerRatio
from source.ml.models.ContrastPhysPlus import ContrastPhysPlus
from source.ml.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


class ContrastPhysPlusTrainer(BaseTrainer):
    def __init__(self, config, data_loader, trainer_params):
        ...
        self.model = ContrastPhysPlus(S=2, in_ch=1).to(self.device)
        if len(device_ids_list) > 1:
            self.model = torch.nn.DataParallel(self.model, device_ids=device_ids_list)

        if config.TOOLBOX_MODE == "train_and_test":
            delta_t = config.TRAIN.DATA.PREPROCESS.CHUNK_LENGTH // 2
            K = 4
            self.loss_model = ContrastLoss(delta_t, K, config.TRAIN.DATA.FS, high_pass=40, low_pass=250)
            self.IPR = IrrelevantPowerRatio(Fs=config.TRAIN.DATA.FS, high_pass=40, low_pass=250)
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                # if idx<1778: continue
                tbar.set_description("Train epoch %s" % epoch)
                label = batch[1][0].squeeze(0).to(self.device)
                data = batch[0][0].squeeze(0).to(self.device)

                model_output = self.model(data)  # Last output

                # Create labels_flag with same dims as labels
                # labels_flag = torch.unsqueeze(torch.Tensor([1] * label.shape[0]), 1).to(self.device)
                # Make labels flag with 0.5 chance to 0 for individual indices
                """labels_flag = torch.zeros(label.shape[0], 1).to(self.device)
                for i in range(label.shape[0]):
                    if np.random.rand() < 0.5:
                        labels_flag[i] = 1
                    else:
                        labels_flag[i] = 0"""
                labels_flag = torch.ones(label.shape[0], 1).to(self.device)

                loss, p_loss, n_loss, p_loss_gt, n_loss_gt = self.loss_model(model_output, label, labels_flag)
    ...
    def valid(self, data_loader):
        ...
        self.model.eval()
        valid_step = 0
        IPR = IrrelevantPowerRatio(Fs=self.config.TRAIN.DATA.FS, high_pass=40, low_pass=250)
        with torch.no_grad():
            if self.config.LABEL_SIGNALS[self.config.LABEL_VALID] in ['ppg_finger', 'ppg_ear', 'ppg_nose', 'hr', 'ppg',
                ...
                for idx, batch in enumerate(vbar):
                    ...
                    labels_flag = torch.ones(label.shape[0], 1).to(self.device)
                    pred = self.model(batch[0][0].squeeze(0).to(self.device))
                    rppg = pred[:, -1]

                    loss, p_loss, n_loss, p_loss_gt, n_loss_gt = self.loss_model(pred, label, labels_flag)
                    valid_loss.append(loss.item())
                    valid_step += 1
                    vbar.set_postfix(loss=loss.item())

                    """ipr = torch.mean(IPR(rppg.clone().detach()))
                    valid_loss.append(ipr.item())
                    valid_step += 1
                    vbar.set_postfix(loss=ipr.item())"""
            else:
