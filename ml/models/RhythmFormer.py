...
class video_BRA(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        # region_size = (t_region, H // 4, W // 4)  # ToDo
        region_size = (t_region, 1, W // 4)
...
class Fusion_Stem(nn.Module):
    def __init__(self, apha=0.5, belta=0.5):
        ...
        self.stem11 = nn.Sequential(nn.Conv2d(1, 64, kernel_size=5, stride=2, padding=2),
                                    nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
                                    nn.ReLU(inplace=True),
                                    nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
                                    )

        self.stem12 = nn.Sequential(nn.Conv2d(4, 64, kernel_size=5, stride=2, padding=2),
                                    nn.BatchNorm2d(64),
                                    nn.ReLU(inplace=True),
                                    nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
                                    )
    ...
    def forward(self, x):
        ...
        x_diff = self.stem12(torch.cat([x2 - x1, x3 - x2, x4 - x3, x5 - x4], 2).view(N * D, 4, H, W))  # ToDo: Replaced 12 by 4
...
class RhythmFormer(nn.Module):
    ...
    def forward(self, x):
        N, D, C, H, W = x.shape
        x = self.Fusion_Stem(x)  # [N*D 64 H/4 W/4]  # [512, 64, 12, 32]
        x = x.view(N, D, 64, H // 4, W // 4).permute(0, 2, 1, 3, 4)  # [4, 64, 128, 12, 32]
        x = self.patch_embedding(x)  # [N 64 D 8 8]  # [4, 64, 128, 3, 8]
        for i in range(3):
